...
| [`v0.4.20+28`](https://github.com/skroll-dev/diary/releases/tag/v0.4.20%2B28) | 2026-08-22 | Fixes a crash on long diary entries (unhandled 422 once a day's dictation passed 8,000 chars); unifies transcript limits at 20,000 chars, proactively blocks recording near the limit, and adds an editable retry dialog on failure instead of failing silently. |
| [`v0.4.22+30`](https://github.com/skroll-dev/diary/releases/tag/v0.4.22%2B30) | 2026-08-22 | Fixes #9 (a new recording on a day that already had an entry silently merged into it instead of becoming its own entry) — a day can now hold unlimited independent entries, each editable/deletable on its own; History groups same-day entries under a date divider and adds a "+" button to start a new entry. |
| [`v0.4.23+31`](https://github.com/skroll-dev/diary/releases/tag/v0.4.23%2B31) | 2026-08-22 | Fixes a regression from v0.4.22+30: past entries could be edited from History, not just today's. Deletion remains available for any entry regardless of date. |
| [`v0.4.24+32`](https://github.com/skroll-dev/diary/releases/tag/v0.4.24%2B32) | 2026-08-25 | Adds photo attachments to diary entries — up to 10 photos per entry, added/reordered/deleted from the entry screen, with a photo strip on History cards and a read-only gallery in the entry detail view. Photos are resized and stripped of EXIF/GPS metadata server-side (new image-proxy backend service) before being stored, isolated per user. |
| [`v0.4.25+33`](https://github.com/skroll-dev/diary/releases/tag/v0.4.25%2B33) | 2026-08-27 | Fixes the fullscreen photo viewer: pinch-to-zoom didn't work on Android (competed with the swipe-between-photos gesture) and double-tap-to-zoom wasn't implemented. Also removes the delete-photo confirmation popup and undo toast — deleting a photo is now instant. |
| [`v0.4.26+34`](https://github.com/skroll-dev/diary/releases/tag/v0.4.26%2B34) | 2026-08-28 | Redesigns the Verlauf (history) entry cards: photo entries show a full-width hero photo with glass mood/photo-count badges instead of a thumbnail row; every card gets a mood-colored accent stripe, and tags use a single consistent accent color instead of cycling through multiple palette colors. |
| [`v0.4.28+36`](https://github.com/skroll-dev/diary/releases/tag/v0.4.28%2B36) | 2026-09-13 | Adds German/English localization throughout the app — every screen is translated, a new Settings screen switches between System/Deutsch/English, and dictation + AI-generated entries respond in the selected language. Moves the "delete all data" danger zone from Profile to Settings. |
